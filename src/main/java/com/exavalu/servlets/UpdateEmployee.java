...
public class UpdateEmployee extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null || user.isLoggedIn()) {
			...
			boolean result = EmployeeService.updateEmploye(employee, propertyValues);
			
			int addressId = EmployeeService.getAddressId(employeeCode, propertyValues);
			
			boolean result2 = EmployeeService.updateAddress(address, addressId, propertyValues);
...
//				request.getRequestDispatcher("pages/addEmployee.jsp").forward(request, response);
				...
				// EmployeeService.getAllActiveEmployees(propertyValues);
